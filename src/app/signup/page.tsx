    ...
    <div style={{
      ...
      <div style={{ width: "100%", maxWidth: 400 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 24 }}>
          <button
            onClick={() => { if (oauthLoading) return; setOauthLoading("google"); signIn("google", { callbackUrl: "/onboarding" }); }}
            disabled={!!oauthLoading}
            style={{
              height: 48, borderRadius: 12, border: `0.5px solid ${T.borderStrong}`,
              background: T.surface, color: T.text, fontSize: 14, fontWeight: 500,
              fontFamily: T.sans, cursor: oauthLoading ? "not-allowed" : "pointer", opacity: oauthLoading && oauthLoading !== "google" ? 0.4 : 1,
              display: "flex", alignItems: "center", justifyContent: "center", gap: 10, letterSpacing: -0.1,
            }}
          >
            ...
            {oauthLoading === "google" ? "연결 중..." : "Google로 가입하기"}
          </button>
          <button
            onClick={() => { if (oauthLoading) return; setOauthLoading("github"); signIn("github", { callbackUrl: "/onboarding" }); }}
            disabled={!!oauthLoading}
            style={{
              height: 48, borderRadius: 12, border: `0.5px solid ${T.borderStrong}`,
              background: T.surface, color: T.text, fontSize: 14, fontWeight: 500,
              fontFamily: T.sans, cursor: oauthLoading ? "not-allowed" : "pointer", opacity: oauthLoading && oauthLoading !== "github" ? 0.4 : 1,
              display: "flex", alignItems: "center", justifyContent: "center", gap: 10, letterSpacing: -0.1,
            }}
          >
            ...
            {oauthLoading === "github" ? "연결 중..." : "GitHub로 가입하기"}
          </button>
        </div>
